...
public class Mundo {
    ...
    static final int INCREMENTO_PUNTUACION_TIPO0 = 10;
    static final int INCREMENTO_PUNTUACION_TIPO1 = 20;
    static final int INCREMENTO_PUNTUACION_TIPO2 = 30;
    ...
    public void update(float deltaTime) {
        ...
        while (tiempoTick > tick) {
            tiempoTick -= tick;
            jollyroger.avance();
            if (jollyroger.comprobarChoque() || jollyroger.comprobarLimite()) {
                finalJuego = true;
                return;
            }
            ...
            if (head.x == botin.x && head.y == botin.y) {
                if (botin.tipo == 0) {
                    puntuacion += INCREMENTO_PUNTUACION_TIPO0;
                } else if (botin.tipo == 1) {
